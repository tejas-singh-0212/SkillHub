...
import { signIn, signInWithGoogle, logOut } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  const sendOtp = useCallback(
    async (targetEmail: string) => {
      ...
      // For demo mode (no SMTP configured), show OTP
      if (data.demoOtp) {
  ...
  const handleBackToCredentials = async () => {
    // Sign out since user may want to try different credentials
    try {
      await logOut();
    } catch {
      // ignore
          ...
          {step === "credentials" ? (
            <>
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 border-2 border-gray-200 dark:border-gray-700 rounded-xl py-3 px-4 hover:bg-gray-50 dark:bg-gray-900 transition font-medium disabled:opacity-50"
              >
            ...
            <div className="animate-fade-in">
              ...
              {/* Demo OTP Banner (when SMTP not configured) */}
              {demoOtp && (
                <div className="mb-4 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg px-4 py-3 text-center">
                  <p className="text-xs text-amber-600 dark:text-amber-400 font-medium">
                    📧 Demo Mode — Your OTP is:
                  </p>
                  <p className="text-2xl font-bold text-amber-700 dark:text-amber-300 tracking-widest mt-1">
                    {demoOtp}
                  </p>
                </div>
              )}
              ...
              >
                {otp.map((digit, index) => (
                  <input
                    ...
                    className={`w-11 h-13 sm:w-13 sm:h-14 text-center text-xl sm:text-2xl font-bold rounded-xl border-2 outline-none transition-all duration-200
                      ${
                        digit
                          ? "border-blue-500 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 shadow-sm shadow-blue-200 dark:shadow-blue-900/50"
              ...
              <button
                ...
                className="w-full bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700 transition disabled:opacity-50 mb-4"
              >
                {loading ? (
