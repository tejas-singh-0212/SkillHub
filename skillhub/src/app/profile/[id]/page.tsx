...
import type { UserProfile, Review, SkillOffered, SkillNeeded } from "@/types";
...
export default function ProfilePage() {
 ...
 const [bookingSkill, setBookingSkill] = useState<SkillOffered | null>(null);
 ...
 setLoading(true);
 try {
 const data = await getUserProfile(profileId as string);
 setProfileData(data);

 const reviewsData = await getReviewsForUser(profileId as string);
 ...
 alert("Error starting conversation: " + (err as Error).message);
 }
 };
 ...
 {profileData.bio && (
 <p className="text-gray-600 dark:text-gray-300 mb-3">{profileData.bio}</p>
 )}
 ...
 className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-6 py-2 rounded-lg hover:bg-gray-300 transition"
 >
 ⚙️ Edit Profile
 </button>
 )}
 ...
 {profileData.skillsOffered.map((skill: SkillOffered, i: number) => (
 <SkillCard
 key={i}
 skill={skill}
 onBook={
 ...
 <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 text-center text-gray-500 dark:text-gray-400">
 ...
 <div className="flex flex-wrap gap-3">
 {profileData.skillsNeeded.map((skill: SkillNeeded, i: number) => {
