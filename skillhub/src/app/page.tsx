    ...
    <div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-20 md:py-32">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-4">
              {!loading && !user ? (
                <>
                  <Link href="/register" className="bg-yellow-400 text-gray-900 px-8 py-4 rounded-xl text-lg font-bold hover:bg-yellow-300 transition shadow-lg">Get Started for Free</Link>
                  <Link href="/search" className="bg-white/10 backdrop-blur text-white px-8 py-4 rounded-xl text-lg font-medium hover:bg-white/20 transition border border-white/30">Explore Skills</Link>
                </>
              ) : !loading && user ? (
                <>
                  <Link href="/search" className="bg-yellow-400 text-gray-900 px-8 py-4 rounded-xl text-lg font-bold hover:bg-yellow-300 transition shadow-lg">Find Skills Near You</Link>
                  <Link href="/dashboard" className="bg-white/10 text-white px-8 py-4 rounded-xl text-lg font-medium hover:bg-white/20 transition border border-white/30">My Dashboard</Link>
                </>
              ) : null}
            </div>
          </div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <p className="text-gray-600 text-center mb-12 text-lg">From tutoring to home repairs, find everything nearby</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {SKILL_CATEGORIES.map((cat) => (
              <Link key={cat.id} href={`/search?category=${cat.id}`} className="bg-white rounded-xl p-5 text-center hover:shadow-lg transition border hover:border-blue-300 group">
                <div className="text-3xl mb-2">{cat.icon}</div>
                <p className="text-sm font-medium text-gray-700 group-hover:text-blue-600">{cat.label}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 bg-gray-50 text-center">
        <div className="max-w-3xl mx-auto px-6">
          ...
          <p className="text-xl text-gray-600 mb-8">Join your local skill exchange community today.</p>
          {!loading && !user && (
            <Link href="/register" className="bg-blue-600 text-white px-10 py-4 rounded-xl text-lg font-bold hover:bg-blue-700 transition inline-block shadow-lg">Create Free Account →</Link>
          )}
        </div>
      </section>
