...
export default function ChatWindow({ conversationId }: ChatWindowProps) {
 ...
 <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-2xl ${msg.senderId === user!.uid ? "bg-blue-600 text-white rounded-br-sm" : "bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-bl-sm"}`}>
 <p>{msg.content}</p>
 <p className={`text-xs mt-1 ${msg.senderId === user!.uid ? "text-blue-200" : "text-gray-500 dark:text-gray-400"}`}>
 ...
 <input value={newMsg} onChange={(e) => setNewMsg(e.target.value)} onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && handleSend()} placeholder="Type a message..." className="flex-1 border dark:border-gray-700 rounded-xl px-4 py-2 outline-none focus:ring-2 focus:ring-blue-500" />
